test_that("peirr_tau estimates gamma exactly as peirr_removal_rate across valid beta/gamma ratios", {
  scenarios <- list(
  ...
  for (idx in seq_along(scenarios)) {
    ...
    # gamma estimate in peirr_tau should match the direct estimator exactly
    expect_equal(tau_fit$removal_rate, gamma_ref)
  }
})

test_that("peirr_tau estimates beta with high tolerance across valid beta/gamma ratios", {
  scenarios <- list(
    list(beta = 1.0, gamma = 1.0),
    list(beta = 1.5, gamma = 1.0),
    list(beta = 2.0, gamma = 1.0),
...
test_that("peirr_tau works with lag > 0 and keeps gamma consistent with peirr_removal_rate", {
  beta_true <- 1.8
  gamma_true <- 1.0
  population_size <- 220
  lag <- 1.5
  ...
  # gamma estimate in peirr_tau should match direct complete-period estimator exactly
  expect_equal(tau_fit$removal_rate, gamma_ref)
...
test_that("peirr_tau works when prop_infection_missing is not 1", {
  beta_true <- 2.2
  gamma_true <- 1.0
  population_size <- 220
  ...
  # gamma estimate in peirr_tau should still match direct complete-period estimator exactly
  expect_equal(tau_fit$removal_rate, gamma_ref)
